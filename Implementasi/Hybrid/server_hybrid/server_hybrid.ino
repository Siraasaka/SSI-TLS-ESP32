...
#include <WiFi.h>
/* Uncomment untuk enable debug log wolfSSL:
   #define DEBUG_WOLFSSL */
#include <wolfssl.h>
#include <wolfssl/ssl.h>
#include <wolfssl/certs_test.h>  /* built-in RSA cert untuk X.509 fallback */
#include <wolfssl/wolfcrypt/ed25519.h>
extern "C" int g_ssi_client_detected; /* hybrid flag dari tls.c library */
extern "C" int g_ssi_client_mode;         /* 1=SSI, 0=X.509 -- dikontrol sketch client */
...
WOLFSSL_CTX* ctx      = NULL;  /* SSI CTX -- Ed25519 key */
WOLFSSL_CTX* ctx_x509 = NULL;  /* X.509 CTX -- RSA cert untuk X.509 client */
WOLFSSL*     ssl      = NULL;
...
static bool setup_wolfssl_ctx(void)
{
    ...
    /* ===================================================================
     * HYBRID MODE: Load RSA cert + key sebagai fallback untuk X.509 client
     *
     * Saat client biasa (tanpa SSI extensions) connect, server perlu
     * punya X.509 cert untuk kirim di Certificate message.
     * Pakai built-in wolfSSL test cert (RSA 2048).
     *
     * Kalau SSI client connect: patch hybrid di library akan skip cert ini
     * dan gunakan Ed25519/VC/DID:key sebagai gantinya.
     * Kalau X.509 client connect: cert ini yang dipakai untuk handshake normal.
     * =================================================================== */
    /* X.509 CTX -- TERPISAH dari ctx SSI agar Ed25519 tidak ter-override */
    ctx_x509 = wolfSSL_CTX_new(wolfTLSv1_3_server_method());
    if (ctx_x509 != NULL) {
        wolfSSL_CTX_set_verify(ctx_x509, WOLFSSL_VERIFY_NONE, NULL);
        wolfSSL_CTX_use_certificate_buffer(ctx_x509,
            server_cert_der_2048, sizeof_server_cert_der_2048,
            WOLFSSL_FILETYPE_ASN1);
        wolfSSL_CTX_use_PrivateKey_buffer(ctx_x509,
            server_key_der_2048, sizeof_server_key_der_2048,
            WOLFSSL_FILETYPE_ASN1);
        wolfSSL_CTX_SetIORecv(ctx_x509,
            [](WOLFSSL* ssl, char* buf, int sz, void* c) -> int {
                WiFiClient* cl = (WiFiClient*)c;
                if (!cl || !cl->connected()) return WOLFSSL_CBIO_ERR_CONN_CLOSE;
                unsigned long t = millis();
                while (cl->available() < 1) {
                    if (millis()-t > 5000) return WOLFSSL_CBIO_ERR_TIMEOUT;
                    delay(1);
                }
                return (int)cl->read((uint8_t*)buf, sz);
            });
        wolfSSL_CTX_SetIOSend(ctx_x509,
            [](WOLFSSL* ssl, char* buf, int sz, void* c) -> int {
                WiFiClient* cl = (WiFiClient*)c;
                if (!cl || !cl->connected()) return WOLFSSL_CBIO_ERR_CONN_CLOSE;
                return (int)cl->write((uint8_t*)buf, sz);
            });
        Serial.println("[TLS] X.509 CTX siap (RSA fallback untuk X.509 client)");
    }
     /*
...
void loop(void)
{
    ...
    /* ===================================================================
     * Buat SSL object untuk koneksi ini (Hybrid mode)
     * =================================================================== */
    g_ssi_client_detected = 0; /* reset sebelum koneksi baru */
    ssl = wolfSSL_new((ctx_x509 != NULL) ? ctx_x509 : ctx);
    if (ssl == NULL) {
     ...
     * =================================================================== */
    /* Hybrid: X.509 client tidak perlu VC exchange */
    if (!g_ssi_client_detected) {
        Serial.println("[Hybrid] X.509 client terdeteksi -- skip VC exchange");
        byte xbuf[64];
        int xn = wolfSSL_read(ssl, xbuf, sizeof(xbuf)-1);
        if (xn > 0) wolfSSL_write(ssl, xbuf, xn);
        wolfSSL_shutdown(ssl);
        wolfSSL_free(ssl); ssl = NULL;
        tcpClient.stop();
        Serial.println("[Server] Koneksi X.509 selesai.");
