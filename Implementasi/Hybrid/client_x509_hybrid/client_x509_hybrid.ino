...
static bool g_repeat = true; /* runtime flag, diset false saat benchmark selesai */

/* Edit this with your other TLS host server address to connect to: */
#define WOLFSSL_TLS_SERVER_HOST "10.138.2.181"
...
/* #define WOLFSSL_SERVER_EXAMPLE */
    ...
    static const char ssid[]     PROGMEM = "Tahu";
    static const char password[] PROGMEM = "merekatahu";
#endif
 ...
 */
#include <wolfssl.h>
extern "C" int g_ssi_client_mode; /* 0=X.509 biasa, 1=SSI */
...
int setup_datetime(void) {
...
#if defined(ESP32)
    /* Coba NTP dulu -- pakai 3 server sebagai fallback */
    configTime(0, 0, "pool.ntp.org", "time.nist.gov", "time.cloudflare.com");

    Serial.print(F("Menunggu NTP"));
    time_t now = time(nullptr);
    int waited = 0;
    /* 1700000000 = Nov 2023 -- cert test wolfSSL valid sampai 2026+ */
    while (now < 1700000000UL && waited < 60) {
        delay(500);
        Serial.print(F("."));
        now = time(nullptr);
        waited++;
    ...
    if (now >= 1700000000UL) {
        ...
        /* NTP gagal -- set manual ke April 2026 */
        Serial.println(F("[NTP] Timeout! Set waktu manual ke April 2026"));
        struct timeval tv;
        tv.tv_sec  = 1743465600UL; /* 1 April 2026 00:00:00 UTC */
        tv.tv_usec = 0;
        settimeofday(&tv, NULL);
        Serial.println(F("[Time] Waktu di-set manual: 2026-04-01"));
    }
#endif
...
void setup(void)
{
    ...
    /* Hybrid: baseline client pakai X.509 biasa, disable SSI extensions */
    g_ssi_client_mode = 0;
...
void loop() {
    ...
    if (reconnect) {
        ...
        if (ret > 0) {
            ...
            unsigned long _hs_start = 0;
            unsigned long _hs_ms = 0;
            do {
                err = 0; /* reset error */
                Serial.println(F("wolfSSL_connect ..."));
                _hs_start = millis();
                ret = wolfSSL_connect(ssl);
                _hs_ms = millis() - _hs_start;
            ...
            /* === BENCHMARK SSI-TLS BASELINE === */
            Serial.print(F("[BM] Handshake: "));
            Serial.print(_hs_ms);
            Serial.println(F(" ms"));
            {
                static int    _n   = 0;
                static unsigned long _sum = 0;
                static unsigned long _min = 0xFFFFFFFFUL;
                static unsigned long _max = 0;
                const int BM_ROUNDS = 10;
                if (_n < BM_ROUNDS) {
                    _sum += _hs_ms;
                    if (_hs_ms < _min) _min = _hs_ms;
                    if (_hs_ms > _max) _max = _hs_ms;
                    _n++;
                    Serial.print(F("Round "));
                    Serial.print(_n);
                    Serial.print(F("/"));
                    Serial.print(BM_ROUNDS);
                    Serial.print(F(" = "));
                    Serial.print(_hs_ms);
                    Serial.println(F(" ms"));
                }
                if (_n == BM_ROUNDS) {
                    Serial.println(F(""));
                    Serial.println(F("=== HASIL BENCHMARK BASELINE TLS 1.3 ==="));
                    Serial.print(F("Rounds  : ")); Serial.println(BM_ROUNDS);
                    Serial.print(F("Mean    : ")); Serial.print(_sum / BM_ROUNDS); Serial.println(F(" ms"));
                    Serial.print(F("Min     : ")); Serial.print(_min); Serial.println(F(" ms"));
                    Serial.print(F("Max     : ")); Serial.print(_max); Serial.println(F(" ms"));
                    Serial.println(F("========================================="));
                    Serial.println(F("[BM] Benchmark selesai. Reset ESP32 untuk ulang."));
                    _n++;
                    g_repeat = false; /* stop loop setelah benchmark selesai */
                }
            }
            /* === END BENCHMARK === */
    ...
    delay(1000);
    if ((reconnect > 0) && (REPEAT_CONNECTION) && g_repeat) {
