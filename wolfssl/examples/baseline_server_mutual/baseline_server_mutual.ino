...
static int _server_round = 0;
#define BM_ROUNDS 150
#if 0
...
int setup_certificates(void)
{
    ...
    /* Require mutual authentication (both sides verify) */
    ...
    /* Load CA cert to verify client certificate (for mutual auth) */
    ret = wolfSSL_CTX_load_verify_buffer(ctx,
                                         CTX_CA_CERT,
                                         CTX_CA_CERT_SIZE,
                                         CTX_CA_CERT_TYPE);
    if (ret == WOLFSSL_SUCCESS) {
        Serial.print("Success: load CA verify cert: ");
        Serial.println(xstr(CTX_CA_CERT));
    }
    else {
...
void loop() {
    ...
    if (client)  {
       ...
       while (!client.connected()) {
        ...
        unsigned long _hs_start = millis();
        ret = wolfSSL_accept(ssl);
        unsigned long _hs_ms = millis() - _hs_start;
        if (ret != WOLFSSL_SUCCESS) {
        ...
        Serial.print(F("[BM] Server handshake: "));
        Serial.print(_hs_ms);
        Serial.println(F(" ms"));
        ...
        wolfSSL_free(ssl);
        _server_round++;
        Serial.print(F("[BM] Round "));
        Serial.print(_server_round);
        Serial.print(F("/"));
        Serial.println(BM_ROUNDS);

        if (_server_round >= BM_ROUNDS) {
            client.stop();
            Serial.println(F(""));
            Serial.println(F("[Server] All 150 rounds done. Halting."));
            while (1) { delay(10000); }
        }
        else {
