...
#define WOLFSSL_TLS_SERVER_HOST "10.138.2.152"

/* wolfssl TLS examples communicate on port 11111 */
#define WOLFSSL_PORT 8443
...
int setup_certificates(void)
{
    ...
    /* Certificate - use server cert (same CA chain as server, guaranteed verify) */
    ...
    /* Setup private key (matching server cert) */
...
#if defined(DEBUG_WOLFSSL)
    /* Debug OFF for benchmark — too much serial output slows everything */
    /* wolfSSL_Debugging_ON(); */
#endif
    ...
    if (reconnect) {
        ...
        if (ret > 0) {
            ...
            Serial.println(F(" ms"));
            {
                ...
                const int BM_ROUNDS = 150;
                static unsigned long _times[150]; /* store all times */
                if (_n < BM_ROUNDS) {
                    _sum += _hs_ms;
                    ...
                    Serial.print(_n);
                    Serial.print(F(","));
                    Serial.println(_hs_ms);
                }
                if (_n == BM_ROUNDS) {
                    float mean = (float)_sum / BM_ROUNDS;
                    float var = 0;
                    for(int _i=0;_i<BM_ROUNDS;_i++){
                        float d=(float)_times[_i]-mean; var+=d*d;
                    }
                    float sd = sqrt(var/BM_ROUNDS);
                    Serial.println(F(""));
                    Serial.println(F("=== HASIL BENCHMARK BASELINE mTLS 1.3 ==="));
                    Serial.print(F("Rounds  : ")); Serial.println(BM_ROUNDS);
                    Serial.print(F("Mean    : ")); Serial.print(mean,1); Serial.println(F(" ms"));
                    Serial.print(F("Min     : ")); Serial.print(_min); Serial.println(F(" ms"));
                    Serial.print(F("Max     : ")); Serial.print(_max); Serial.println(F(" ms"));
                    Serial.print(F("StdDev  : ")); Serial.print(sd,1); Serial.println(F(" ms"));
                    Serial.println(F("========================================="));
                    Serial.println(F("[BM] Benchmark selesai. Halting."));
                    _n++;
                    g_repeat = false;
                    /* Hard halt */
                    while(1) { delay(10000); }
