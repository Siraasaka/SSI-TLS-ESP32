    ...
    switch(ssl->options.asyncState)
    {
        ...
        case TLS_ASYNC_BUILD:
        {
            ...
            if (ssl->buffers.key == NULL) {
                ...
                if (ssl->options.ssiVCMode &&
                    ssl->hsType == DYNAMIC_TYPE_ED25519 &&
                    ssl->hsKey != NULL) {
                    /* SSI-TLS: Ed25519 key already loaded by
                     * wolfSSL_SSI_use_Ed25519_key, skip DecodePrivateKey */
                    args->sigLen = ED25519_SIG_SIZE;
                    WOLFSSL_MSG("SSI-TLS: using pre-loaded Ed25519 key");
                }
                else
#endif /* WOLFSSL_SSI_TLS */
                {
                    ret = DecodePrivateKey(ssl, &args->sigLen);
                    if (ret != 0)
                        goto exit_scv;
                }
            } /* closes else { from "if (ssl->buffers.key == NULL)" */
