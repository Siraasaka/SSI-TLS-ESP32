...
/* Check if any non-X.509 cert type (RPK or VC) is in the list.
 * Used to decide whether to send cert type extensions. */
static int IsCertTypeNonX509Listed(byte cnt, const byte* list)
{
    if (IsCertTypeListed(WOLFSSL_CERT_TYPE_RPK, cnt, list))
        return 1;
    if (IsCertTypeListed(WOLFSSL_CERT_TYPE_VC, cnt, list))
        return 1;
    return 0;
}
#endif
...
static int TLSX_ClientCertificateType_Use(WOLFSSL* ssl, byte isServer)
{
    ...
    if (isServer) {
...
#if defined(WOLFSSL_SSI_TLS)
            || IsCertTypeListed(WOLFSSL_CERT_TYPE_VC,
                        ssl->options.rpkConfig.preferred_ClientCertTypeCnt,
                        ssl->options.rpkConfig.preferred_ClientCertTypes)
#endif
            ) {
        ...
        /* [in client side]
         * This extension MUST be omitted from the ClientHello unless a
         * non-X.509 certificate type is preferred and actually loaded.
         */
        ...
        if (IsCertTypeListed(WOLFSSL_CERT_TYPE_RPK,
            ...
            if (ssl->options.rpkState.isRPKLoaded) {
                ...
                /* Send full preference list (may include VC, RPK, X509) */
                ssl->options.rpkState.sending_ClientCertTypeCnt =
                    ssl->options.rpkConfig.preferred_ClientCertTypeCnt;
                XMEMCPY(ssl->options.rpkState.sending_ClientCertTypes,
                         ssl->options.rpkConfig.preferred_ClientCertTypes,
                         ssl->options.rpkConfig.preferred_ClientCertTypeCnt);
                ...
                WOLFSSL_MSG("Willing to use RPK/VC cert but not loaded it");
            }
        }
        else {
            WOLFSSL_MSG("No will to use RPK/VC cert");
        }
    }
    return ret;
}
...
static int TLSX_ServerCertificateType_Use(WOLFSSL* ssl, byte isServer)
{
    ...
    if (isServer) {
        ...
        if (IsCertTypeListed(WOLFSSL_CERT_TYPE_RPK,
            ...
            WOLFSSL_MSG("No will to accept RPK/VC cert");
        }
    }
...
int TLSX_PopulateExtensions(WOLFSSL* ssl, byte isServer)
{
    ...
    if (!isServer) {
        ...
        /* If client configured VC(3) in server cert type preferences,
         * prepare did_methods extension data for ClientHello */
        {
            int _i;
            int _wantVC = 0;
            for (_i = 0; _i < ssl->options.rpkConfig.preferred_ServerCertTypeCnt;
                 _i++) {
                if (ssl->options.rpkConfig.preferred_ServerCertTypes[_i] ==
                    WOLFSSL_CERT_TYPE_VC) {
                    _wantVC = 1;
                    break;
                }
            }
            if (_wantVC) {
                ...
                WOLFSSL_MSG("SSI-TLS: client will send did_methods in CH");
            }
        }
...
#if defined(WOLFSSL_SSI_TLS) && defined(HAVE_RPK)
    /* Account for did_methods extension size in ClientHello */
    if (msgType == client_hello && ssl->options.ssiSendDIDMethods) {
        /* 2B type + 2B length + 1B count + N method bytes */
        length += HELLO_EXT_TYPE_SZ + HELLO_EXT_SZ_SZ +
                  1 + ssl->options.ssiDIDMethodsTxCount;
    }
#endif
...
#if defined(WOLFSSL_SSI_TLS) && defined(HAVE_RPK)
    /* Append did_methods extension (0xFF02) to ClientHello if SSI mode */
    if (msgType == client_hello && ssl->options.ssiSendDIDMethods) {
        byte dmCount = ssl->options.ssiDIDMethodsTxCount;
        word16 extDataLen = 1 + dmCount; /* 1B count + method bytes */

        /* Extension type: 0xFF02 */
        c16toa(0xFF02, output + offset);
        offset += HELLO_EXT_TYPE_SZ;
        /* Extension data length */
        c16toa(extDataLen, output + offset);
        offset += HELLO_EXT_SZ_SZ;
        /* Payload */
        *(output + offset) = dmCount;
        ...
        WOLFSSL_MSG("SSI-TLS: wrote did_methods in ClientHello");
    }
#endif /* WOLFSSL_SSI_TLS && HAVE_RPK */
...
#if defined(WOLFSSL_SSI_TLS) && defined(HAVE_RPK)
    /* Account for did_methods extension size in EncryptedExtensions */
    if (msgType == encrypted_extensions && ssl->options.ssiNegotiated) {
        length += HELLO_EXT_TYPE_SZ + HELLO_EXT_SZ_SZ +
                  1 + ssl->options.ssiDIDMethodsRxCount;
    }
#endif
...
#if defined(WOLFSSL_SSI_TLS) && defined(HAVE_RPK)
        /* Server: append did_methods in EncryptedExtensions if SSI negotiated */
        if (msgType == encrypted_extensions && ssl->options.ssiNegotiated) {
            byte dmCount = ssl->options.ssiDIDMethodsRxCount;
            word16 extDataLen = 1 + dmCount;
            ...
            *(output + offset) = dmCount;
            ...
            WOLFSSL_MSG("SSI-TLS: wrote did_methods in EncryptedExtensions");
        }
#endif /* WOLFSSL_SSI_TLS && HAVE_RPK */
...
#endif
#if defined(WOLFSSL_SSI_TLS) && defined(HAVE_RPK)
            case 0xFF02: /* TLSX_DID_METHODS — private-use extension
                          * (draft-vesco-vcauthtls-01) */
                WOLFSSL_MSG("did_methods extension received");
                if (msgType == client_hello) {
                    /* Server: parse client's DID method list */
                    if (size >= 1) {
                        ...
                        if (dmCount > 0 && dmCount <= 8 &&
                            ...
                            WOLFSSL_MSG("SSI-TLS: did_methods from client OK");
                        }
                    }
                }
                else if (msgType == encrypted_extensions) {
                    /* Client: server confirmed SSI mode */
                    if (size >= 1) {
                        byte dmCount = input[offset];
                        if (dmCount > 0 && dmCount <= 8 &&
                            (word16)(1 + dmCount) <= size) {
                            ...
                            XMEMCPY(ssl->options.ssiDIDMethodsRx,
                                    input + offset + 1, dmCount);
                            ssl->options.ssiNegotiated = 1;
                            WOLFSSL_MSG("SSI-TLS: server confirmed SSI mode");
                        }
                    }
                }
                break;
