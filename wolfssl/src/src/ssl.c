...
#if defined(WOLFSSL_SSI_TLS)
            && buf[i] != WOLFSSL_CERT_TYPE_VC
#endif
            )
            return BAD_FUNC_ARG;
...
#if defined(WOLFSSL_SSI_TLS) && defined(HAVE_RPK) && defined(HAVE_ED25519)

/* Load VC bytes as the "certificate" for SSI-TLS handshake.
 * This directly copies raw VC binary data into ssl->buffers.certificate,
 * bypassing ProcessBuffer (which expects X.509 or RPK SubjectPublicKeyInfo).
 *
 * The VC bytes will be sent in the TLS 1.3 Certificate message as-is.
 * The peer (also running SSI-TLS) will parse it using SSI_VC_ParseAndVerify.
 *
 * @param ssl    SSL object
 * @param vc     VC binary data (from SSI_Identity_CreateVC)
 * @param vcSz   Size of VC data in bytes
 * @return WOLFSSL_SUCCESS on success, negative error code on failure
 */
int wolfSSL_SSI_use_VC_buffer(WOLFSSL* ssl, const unsigned char* vc,
                              unsigned int vcSz)
{
    int ret;

    WOLFSSL_ENTER("wolfSSL_SSI_use_VC_buffer");

    if (ssl == NULL || vc == NULL || vcSz == 0)
        return BAD_FUNC_ARG;

    /* Free existing certificate if any */
    FreeDer(&ssl->buffers.certificate);

    /* Allocate and copy VC bytes into certificate buffer */
    ret = AllocDer(&ssl->buffers.certificate, vcSz, CERT_TYPE, ssl->heap);
    if (ret != 0) {
        WOLFSSL_MSG("SSI-TLS: failed to allocate cert buffer for VC");
        return ret;
    }

    XMEMCPY(ssl->buffers.certificate->buffer, vc, vcSz);
    ssl->buffers.weOwnCert = 1;

    /* Clear cert chain — VC has no chain */
    FreeDer(&ssl->buffers.certChain);
    ssl->buffers.certChainCnt = 0;

    /* Mark RPK state so wolfSSL knows this is not X.509 */
    ssl->options.rpkState.isRPKLoaded = 1;

    /* Mark VC mode */
    ssl->options.ssiVCMode = 1;

    WOLFSSL_MSG("SSI-TLS: VC loaded as certificate buffer");

    return WOLFSSL_SUCCESS;
}

/* Load Ed25519 private key for SSI-TLS CertificateVerify signing.
 * The key is a raw 32-byte Ed25519 seed (private key).
 *
 * This creates an ed25519_key, imports the seed + derives pubkey,
 * and sets ssl->hsKey / ssl->hsType so SendTls13CertificateVerify
 * can use it for Ed25519 signing.
 *
 * @param ssl     SSL object
 * @param privKey Raw 32-byte Ed25519 private key (seed)
 * @param pubKey  Raw 32-byte Ed25519 public key
 * @return WOLFSSL_SUCCESS on success, negative error code on failure
 */
int wolfSSL_SSI_use_Ed25519_key(WOLFSSL* ssl, const unsigned char* privKey,
                                const unsigned char* pubKey)
{
    int ret;
    ed25519_key* key;

    WOLFSSL_ENTER("wolfSSL_SSI_use_Ed25519_key");

    if (ssl == NULL || privKey == NULL || pubKey == NULL)
        return BAD_FUNC_ARG;

    /* Free existing handshake key if any */
    FreeKey(ssl, ssl->hsType, (void**)&ssl->hsKey);

    /* Allocate new Ed25519 key */
    ret = AllocKey(ssl, DYNAMIC_TYPE_ED25519, (void**)&ssl->hsKey);
    if (ret != 0) {
        WOLFSSL_MSG("SSI-TLS: failed to allocate Ed25519 key");
        return ret;
    }

    key = (ed25519_key*)ssl->hsKey;

    /* Import private + public key */
    ret = wc_ed25519_import_private_key(privKey, ED25519_KEY_SIZE,
                                        pubKey, ED25519_PUB_KEY_SIZE,
                                        key);
    if (ret != 0) {
        WOLFSSL_MSG("SSI-TLS: Ed25519 key import failed");
        FreeKey(ssl, DYNAMIC_TYPE_ED25519, (void**)&ssl->hsKey);
        return ret;
    }

    ssl->hsType = DYNAMIC_TYPE_ED25519;
    ssl->options.haveECDSAsig = 1;  /* Ed25519 shares the ECDSA flag */
    ssl->buffers.keyType = ed25519_sa_algo;
    ssl->buffers.keySz = ED25519_KEY_SIZE;

    /* Also store in buffers.key for PK callbacks */
    FreeDer(&ssl->buffers.key);
    ret = AllocDer(&ssl->buffers.key, ED25519_KEY_SIZE + ED25519_PUB_KEY_SIZE,
                   PRIVATEKEY_TYPE, ssl->heap);
    if (ret == 0) {
        XMEMCPY(ssl->buffers.key->buffer, privKey, ED25519_KEY_SIZE);
        XMEMCPY(ssl->buffers.key->buffer + ED25519_KEY_SIZE,
                 pubKey, ED25519_PUB_KEY_SIZE);
        ssl->buffers.weOwnKey = 1;
    }

    /* Re-initialize cipher suites now that we have key type info.
     * This is needed because InitSSL_Suites ran during wolfSSL_new
     * with keySz=0 and haveECDSAsig=0, so TLS 1.3 suites may be
     * missing from the list. */
    if (ssl->suites != NULL) {
        ret = InitSSL_Suites(ssl);
        if (ret != WOLFSSL_SUCCESS) {
            WOLFSSL_MSG("SSI-TLS: InitSSL_Suites re-init failed");
            return ret;
        }
    }

    WOLFSSL_MSG("SSI-TLS: Ed25519 key loaded for CertificateVerify");

    return WOLFSSL_SUCCESS;
}

#endif /* WOLFSSL_SSI_TLS && HAVE_RPK && HAVE_ED25519 */
